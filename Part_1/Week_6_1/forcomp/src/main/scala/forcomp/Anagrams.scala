...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (w.toLowerCase groupBy (c => c) map { case (ltr, occ) => (ltr, occ.length) } toList).sortBy { case (ltr, _) => ltr }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def sub(subSet: Map[Char, Int])(c: Char, n: Int): (Char, Int) = {
      (c, n - subSet(c))
    }

    val ySub: (Char, Int) => (Char, Int) = sub(y.toMap withDefaultValue 0)

    (for {
      occ <- x map { case (c, n) => ySub(c, n) }
      if occ._2 > 0
    } yield occ).sortBy { case (ltr, _) => ltr }
  }
